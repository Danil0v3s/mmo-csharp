...
public enum PacketHeader : short
{
    ...
    ZC_ACTION_FAILURE = 0x013b,            // clif_arrow_fail — no/wrong ammo (caster-only). <type>.W (4B).
    ZC_SKILL_CAST_CANCEL = 0x01b9,         // clif_skillcastcancel — clears the cast bar. <GID>.L (6B).
    ...
    CZ_REMEMBER_WARPPOINT = 0x011d,        // clif_parse_RequestMemo — "remember this point" → pc_memo(sd,-1). 2B header only.
    ...
    ZC_ALL_QUEST_LIST     = 0x09f8,        // clif_quest_send_list (login snapshot): <len>.W <count>.L then per-quest 15B + per-obj 44B (incl target+current+name).
    ...
    CZ_REQ_READ_MAIL      = 0x09ea,        // RODEX read one mail (clif_parse_Mail_read): opentype.B mailID.Q, 11B
    ZC_ACK_READ_RODEX     = 0x09eb,        // RODEX read-window (clif_Mail_read, PACKETVER ≥ 20150513), variable
    ...
    ZC_MONSTER_INFO       = 0x018c,        // clif_skill_estimation (Sense / BS_GREED estimation), 29B fixed
    ZC_MAKABLEITEMLIST    = 0x025a,        // clif_cooking_list (Pharmacy / Cooking dialog), variable
