...
public sealed class StatusEffectRegistry
{
    ...
    public StatusEffectRegistry()
    {
        ...
        Register(StatusType.Curse, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Debuff | ScfFlag.RemoveOnRefresh,
            // SC-DERIVED-RECALC — re-apply reset field(s) after CalcPc rebuild.
            OnRecalc: (target, sc) =>
            {
                target.Stats.Batk = (ushort)Math.Max(0, target.Stats.Batk - sc.Val3);
            }));
        ...
        Register(StatusType.Berserk, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Buff | ScfFlag.RemoveOnLogout,
            // SC-DERIVED-RECALC — the Def/Flee/Batk derived re-fold. CalcPc rebuilds Batk/Flee/Def/Def2/
            // Mdef/Mdef2 each recalc, wiping Berserk's +200 Batk, half-Flee, and zeroed defences. Re-apply
            // them, mirroring OnStart's snapshot packing (so OnEnd still restores the rebuilt base). AspdRate
            // is not reset; MaxHp is handled by OnRecalcPool; the one-time HP-fill is not re-applied.
            OnRecalc: (target, sc) =>
            {
                target.Stats.Batk = (ushort)Math.Min(ushort.MaxValue, target.Stats.Batk + 200);
                int fleeSnap = target.Stats.Flee / 2;
                ushort defSnap = (ushort)Math.Clamp(target.Stats.Def, 0, ushort.MaxValue);
                ushort def2Snap = (ushort)Math.Clamp(target.Stats.Def2, 0, ushort.MaxValue);
                ushort mdefSnap = (ushort)Math.Clamp(target.Stats.Mdef, 0, ushort.MaxValue);
                ushort mdef2Snap = (ushort)Math.Clamp(target.Stats.Mdef2, 0, ushort.MaxValue);
                sc.Val3 = (defSnap << 16) | def2Snap;
                sc.Val4 = (fleeSnap & 0xFFFF) | ((mdefSnap & 0xFF) << 24) | ((mdef2Snap & 0xFF) << 16);
                target.Stats.Flee = (short)Math.Max(0, target.Stats.Flee - fleeSnap);
                target.Stats.Def = 0;
                target.Stats.Def2 = 0;
                target.Stats.Mdef = 0;
                target.Stats.Mdef2 = 0;
            },
            // COMBAT-90 — re-apply the ×3 MaxHp on the rebuilt pool; re-store the ×2 snapshot.
            // Berserk's HP-fill is a one-time cast effect, NOT re-applied on recalc — the CalcPc
            // Hp re-clamp keeps current Hp within the new max.
            OnRecalcPool: (target, sc) =>
            {
                sc.Val2 = target.Stats.MaxHp * 2;
                target.Stats.MaxHp += sc.Val2;
            }));
        ...
        Register(StatusType.DragonicAura, new StatusEffectHandler(
            ...
            OnRecalc: (target, sc) =>
            {
                target.Stats.Patk = (short)Math.Min(short.MaxValue, target.Stats.Patk + sc.Val2);
                target.Stats.Hit = (short)Math.Min(short.MaxValue, target.Stats.Hit + sc.Val3);
            }));
    ...
    private void RegisterWave32Val2Val3Formulas()
    {
        ...
        Register(StatusType.Service4u, new StatusEffectHandler(
            ...
            Flags: buff,
            // COMBAT-90 — re-apply the MaxSp % on the rebuilt pool; re-store the snapshot.
            OnRecalcPool: (target, sc) =>
            {
                var maxSpDelta = target.Stats.MaxSp * sc.Val2 / 100;
                sc.Val4 = maxSpDelta;
        ...
        Register(StatusType.GtChange, new StatusEffectHandler(
            ...
            Flags: buff,
            // SC-DERIVED-RECALC — CalcPc rebuilds Batk; re-apply the % on the rebuilt base + re-snapshot
            // (AspdRate is not reset, leave it).
            OnRecalc: (target, sc) =>
            {
                var batkDelta = target.Stats.Batk * sc.Val2 / 100;
                sc.Val4 = batkDelta;
        ...
        Register(StatusType.Appleidun, new StatusEffectHandler(
            ...
            Flags: buff,
            // COMBAT-90 — re-apply the MaxHp % on the rebuilt pool; re-store the snapshot.
            OnRecalcPool: (target, sc) =>
            {
                var maxHpDelta = target.Stats.MaxHp * sc.Val2 / 100;
                sc.Val4 = maxHpDelta;
        ...
        Register(StatusType.Neutralbarrier, new StatusEffectHandler(
            ...
            Flags: buff,
            // SC-DERIVED-RECALC — CalcPc rebuilds Def+Mdef; re-apply the % on the rebuilt base + re-snapshot.
            OnRecalc: (target, sc) =>
            {
                var defDelta = (short)(target.Stats.Def * sc.Val2 / 100);
                var mdefDelta = (short)(target.Stats.Mdef * sc.Val2 / 100);
        ...
        Register(StatusType.FirmFaith, new StatusEffectHandler(
            ...
            Flags: buff,
            // SC-DERIVED-RECALC — the Res derived re-fold (CalcPc rebuilds Res each recalc).
            OnRecalc: (target, sc) =>
                target.Stats.Res = (short)Math.Min(short.MaxValue, target.Stats.Res + sc.Val3),
            // COMBAT-73 — re-apply the MaxHp % on the rebuilt pool.
            OnRecalcPool: (target, sc) =>
            {
                var hpDelta = target.Stats.MaxHp * sc.Val2 / 100;
                sc.Val4 = hpDelta;
    ...
    private void RegisterWave4aBespokeFormulas()
    {
        ...
        Register(StatusType.Truesight, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Buff | ScfFlag.RemoveOnLogout,
            // SC-DERIVED-RECALC — CalcPc rebuilds Cri+Hit; re-apply (the +5 base stats ride the COMBAT-10
            // param-base delta, so they're NOT re-applied here).
            OnRecalc: (target, sc) =>
            {
                target.Stats.Cri = (short)Math.Min(short.MaxValue, target.Stats.Cri + sc.Val2);
                target.Stats.Hit = (short)Math.Min(short.MaxValue, target.Stats.Hit + sc.Val3);
        ...
        Register(StatusType.Magicpower, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Buff | ScfFlag.RemoveOnLogout,
            // SC-DERIVED-RECALC — CalcPc rebuilds Smatk; re-apply the % on the rebuilt base + re-snapshot.
            OnRecalc: (target, sc) =>
            {
                var delta = (short)(target.Stats.Smatk * (5 * sc.Val1) / 100);
                sc.Val3 = delta;
                target.Stats.Smatk = (short)Math.Min(short.MaxValue, target.Stats.Smatk + delta);
            }));
    ...
    private void RegisterP0Wave2BespokeFormulas()
    {
        ...
        Register(StatusType.Leradsdew, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Buff | ScfFlag.RemoveOnLogout,
            // COMBAT-90 — re-apply the MaxHp % (Val3) on the rebuilt pool. OnEnd recomputes its
            // own inverse delta, so there is no snapshot to store.
            OnRecalcPool: (target, sc) =>
                target.Stats.MaxHp = Math.Min(int.MaxValue / 2, target.Stats.MaxHp + target.Stats.MaxHp * sc.Val3 / 100)));
        ...
        Register(StatusType.Melodyofsink, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Debuff | ScfFlag.RemoveOnRefresh,
            // COMBAT-90 — re-apply the MaxSp -Val3% reduction on the rebuilt pool (the Int debuff
            // survives via the COMBAT-10 param-base delta). Re-clamp Sp to the lowered max.
            OnRecalcPool: (target, sc) =>
            {
                var spDelta = target.Stats.MaxSp * sc.Val3 / 100;
                target.Stats.MaxSp = Math.Max(1, target.Stats.MaxSp - spDelta);
                if (target.Stats.Sp > target.Stats.MaxSp) target.Stats.Sp = target.Stats.MaxSp;
            }));
        ...
        Register(StatusType.Competentia, new StatusEffectHandler(
            ...
            Flags: ScfFlag.Buff | ScfFlag.RemoveOnLogout,
            // SC-DERIVED-RECALC — re-apply the derived stat after CalcPc rebuild.
            OnRecalc: (target, sc) =>
            {
                target.Stats.Patk = (short)Math.Min(short.MaxValue, target.Stats.Patk + sc.Val2);
                target.Stats.Smatk = (short)Math.Min(short.MaxValue, target.Stats.Smatk + sc.Val2);
        ...
        Register(StatusType.MusicalInterlude, new StatusEffectHandler(
            ...
            OnRecalc: (target, sc) =>
            {
                target.Stats.Res = (short)Math.Min(short.MaxValue, target.Stats.Res + sc.Val2);
            }));
        ...
        Register(StatusType.TemporaryCommunion, new StatusEffectHandler(
            ...
            OnRecalc: (target, sc) =>
            {
                target.Stats.Patk = (short)Math.Min(short.MaxValue, target.Stats.Patk + sc.Val2);
                target.Stats.Smatk = (short)Math.Min(short.MaxValue, target.Stats.Smatk + sc.Val2);
                target.Stats.Hplus = (short)Math.Min(short.MaxValue, target.Stats.Hplus + sc.Val2);
            }));
    ...
    private void RegisterWave61BespokeGeneratorOverrides()
    {
        ...
        Register(StatusType.PromoteHealthReserch, new StatusEffectHandler(
            ...
            Flags: buff,
            // COMBAT-90 — re-add the flat MaxHp (Val3) on the rebuilt pool.
            OnRecalcPool: (target, sc) =>
                target.Stats.MaxHp = Math.Max(1, target.Stats.MaxHp + sc.Val3)));
        ...
        Register(StatusType.EnergyDrinkReserch, new StatusEffectHandler(
            ...
            Flags: buff,
            // COMBAT-90 — re-apply the MaxSp % (Val3) on the rebuilt pool (≥1 delta, mirrors
            // OnStart). OnEnd recomputes its own inverse, so there is no snapshot to store.
            OnRecalcPool: (target, sc) =>
            {
                var delta = target.Stats.MaxSp * sc.Val3 / 100;
                if (delta < 1) delta = 1;
        ...
        Register(StatusType.Deluge, new StatusEffectHandler(
            ...
            Flags: buff,
            // COMBAT-90 — re-apply the MaxHp % (Val2) on the rebuilt pool. OnEnd recomputes its
            // own inverse delta, so there is no snapshot to store.
            OnRecalcPool: (target, sc) =>
