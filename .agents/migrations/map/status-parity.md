...
| `status_check_visibility` | ✅ | Wave 81 — `EntityActionGates.CanSee` (status.cpp:2292). Reads 9 hide-class SCs (Hiding, Cloaking, Chasewalk2, Camouflage, Stealthfield, Suhide, Cloakingexceed, Newmoon, Feintbomb, ElementalVeil) + honors source `MobMode.Mvp` (boss-class) and `MobMode.Detector`. Base hide is pierced by boss OR detector; perfect-hide overlay (Cloakingexceed/Newmoon) is pierced by boss only; ElementalVeil + Feintbomb pierced by boss OR detector. Distance gating + SC_BLIND view-range clamp remain on the caller. |
