...
- **Teleport (`pc_setpos`) — still pending.** [`WarpDispatcher`](../../../Map.Server/Warps/WarpDispatcher.cs) currently logs the destination and clears `entity.Walk`; the full `pc_setpos` cascade (saved-coord update, cross-server map re-auth, `ZC_NPCACK_MAPMOVE` for cross-map / `ZC_TELEPORT` for same-map, view-range clear + STANDENTRY of new view) is its own slice. **The warp uses the exact `dst_x, dst_y` from the row** ([npc.cpp:1905](/Volumes/1TB/Projetos/rathena/src/map/npc.cpp) `pc_setpos(sd, …, nd->u.warp.x, nd->u.warp.y, CLR_OUTSIGHT)`); no randomization on the warp path. The random walkable-cell logic in `pc_setpos` ([pc.cpp:6948-6967](/Volumes/1TB/Projetos/rathena/src/map/pc.cpp)) is a *safety net*: it triggers only when (a) the requested coords are out of bounds, or (b) the caller passes (0, 0) as a "pick anywhere" sentinel (used by `@warp <map>` with no coords and by save-point spawn). It is **not** the warp path. `WantToConnectionHandler.PickRandomWalkableCell` already implements this safety branch for the char-select spawn; when `SetposService` lands it should keep this branch for the (0,0)/OOB cases but route the warp call through the exact-coord path.
...
3. **`npc_touch_area_allnpc`** ([npc.cpp:1924](/Volumes/1TB/Projetos/rathena/src/map/npc.cpp)) iterates `mapdata->npc[]` for that map (warps sorted first), and for each calls `npc_touch_areanpc`:
   ...
   - For `NPCTYPE_WARP`: applies the gates (hidden/dead can't warp, job-can-enter-map, rewarp-loop counter ≤ 10) and calls `pc_setpos(sd, nd->u.warp.mapindex, nd->u.warp.x, nd->u.warp.y, CLR_OUTSIGHT)` (npc.cpp:1905) — **exact predefined coordinates from the warp row, not randomized**.
   - For `NPCTYPE_SCRIPT`: fires the OnTouch event.

> Where the random walkable-cell logic actually lives: inside `pc_setpos` itself ([pc.cpp:6948-6967](/Volumes/1TB/Projetos/rathena/src/map/pc.cpp)) — but only as a **safety branch** when the requested `(x, y)` is out of bounds (logged as an error) or is the sentinel `(0, 0)` ("pick anywhere"). The sentinel path is used by `@warp <map>` with no coords and by save-point spawn after corrupted saved coordinates; the warp dispatch never hits it for valid warp data.
