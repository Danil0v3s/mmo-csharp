...
- **2026-05-17** — **Status-broadcast cascade landed.** `StatusBroadcaster.BroadcastStatusCalcFirst` invoked from `WantToConnectionHandler` (after `ZC_NPCACK_MAPMOVE`) mirrors rAthena's `status_calc_pc(SCO_FIRST)` diff-emit byte-for-byte. `StatusBroadcaster.BroadcastLoadEndAck` invoked from `NotifyActorInitHandler` mirrors `clif_parse_LoadEndAck`. `MapSessionData.CharacterData` caches the IPC response between handlers. See [initial-status-broadcast.md](initial-status-broadcast.md) for the full enumeration + [replay-baseline.md](replay-baseline.md) for current diff state (19 content-placeholder diffs across the whole capture).
- **2026-05-17** — **`pc_authok` packet order matches rAthena.** Captured rAthena `pc_authok` emits ZC_AID → ZC_EXTEND_BODYITEM_SIZE → ZC_ACCEPT_ENTER_ZONE → ZC_FRIENDS_LIST → version chat → MOTD lines → ZC_NPCACK_MAPMOVE. Now wired in [WantToConnectionHandler](../../../Map.Server/Handlers/WantToConnectionHandler.cs). New packet classes: [ZC_EXTEND_BODYITEM_SIZE](../../../Core.Server/Packets/Out/ZC/ZC_EXTEND_BODYITEM_SIZE.cs), [ZC_FRIENDS_LIST](../../../Core.Server/Packets/Out/ZC/ZC_FRIENDS_LIST.cs). New config knobs in [MapServerConfiguration](../../../Map.Server/MapServerConfiguration.cs): `DisplayVersion`, `VersionMessage`, `MotdLines` (defaults match rAthena). Also: `pc_setpos` OOB/non-walkable randomize in WantToConnectionHandler — character coords clamped to a random walkable cell when saved coords don't fit the loaded map (e.g. captured `iz_int03,103,89` on the 80×80 map). All packets covered by replay assertion at [replay-baseline.md](replay-baseline.md).
