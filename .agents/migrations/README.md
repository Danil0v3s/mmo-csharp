...
| [Map (declarative catalogs)](map/declarative-catalogs.md) | ✅ Data seeded + warp trigger detection live | 1,279 warps / 2,950 mob spawns / 2,251 mapflags ported from rAthena npc/re/. Warp cells flagged with `NpcTrigger` and detected on tile-step arrival; `pc_setpos` teleport still pending. Spawn loader + mapflag applier pending. |
...
- **2026-05-17** — **Warp trigger detection shipped.** Cell-flag dispatch ported from rAthena — `CellFlags.NpcTrigger` set per-cell at boot via `WarpService` from the seeded `warp` table; `MovementService` checks O(1) on tile arrival. Actual teleport (`pc_setpos`) is the next slice. See [map/declarative-catalogs.md](map/declarative-catalogs.md).
