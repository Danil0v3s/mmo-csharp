...
| [Map (MS3 combat / status / skills / mob AI)](map/adjacent/combat.md) | ✅ Foundation 100% / 🔁 Long-tail data | BattleStats + renewal damage calc, auto-attack loop, mob AI, status changes (5 SCs), skill cast + ground units (8 skills), EXP / level / party share, equip → stats end-to-end, pet/summon AI, sit/stand, items use, stat & skill allocation, trade / shop / storage services + wire packets end-to-end, public chat. Strategy-pattern dispatch in skill resolution, item effects, request-action handler, mob-skill conditions. **309 tests green.** Wire packets for the data side of skill_db / item Scripts and the long tail (drop, whisper/party/guild chat, mail, quest, …) are the remaining work — service contracts are stable. See [combat.md history](map/adjacent/combat.md#history). |
