...
- **2026-05-24** — **P2.2 + P2.3 landed; P2.1 in flight.** Commits `767d9a1` + `7810db5`. PathService PathSearch/PathSearchLong/BlownPos now delegate to the canonical `Pathfinder` A* + Bresenham LoS + walkable-aware halt (was three `return true;` stubs). All 45 inline `data-pending` markers across 13 production files rewritten as `deferred per PARITY-REMAINING.md §P2.2.<x>` with sub-section citations. `grep -rn data-pending Map.Server` returns 0. P2.1 doc-resync sweep (3 parallel agents on 36 `*-parity.md` docs to flip stale ⚠️ rows to ✅ with citation) running concurrently.

- **2026-05-24** — **P1.1 + P1.2 landed.** Commits `0359898` (240 → 0 `// TODO` markers across 16 skill family directories) + `093e6a1` (doc state-of-play) + `6572c6b` (1,675 → 0 `.rathena-todo.txt` advisory files; advisory-emit replaced with stale-file cleanup at test time after RecordingStatusOpsService + percent_heal/percent_damage extractor entries + move-pos↔blow normalization land).

- **2026-05-24** — **P0 + P0 follow-up landed.** Commits `a44d2f9` (15 helpers, 25 bespoke SC formulas, 9 SC consumer reads, sc_start wire, isDisabledOnMap) + `2f07b79` (SkillBehaviorContext threading). PARITY-REMAINING.md re-baselined as a phase-ordered worklist (P0 → P1 → P2).
...
- **2026-05-23** — **AT-E — TRUE 100% across every atcommand parent subsystem (1 commit).** Single commit `<this>` closes the residual ⚠️/❌ entries left by AT-D across all 6 parent subsystems where AT-D shipped real bodies with named data-pending gates. Each gate replaced by inline-baked rAthena reference data (no remaining "loader pending" lines):
  ...
  - **Channel** (26/1/0 → 28/0/0): `ReadConfig` replaced "loader pending" with a `DefaultChannels[]` table mirroring rAthena conf/channels.conf (main/map/guild/trade/system with their type + color); ctor now calls `ReadConfig()` so the seeded set is consistent.
  - **Homunculus** (35/4/0 → 39/0/0): Baked `HomunSkillTree` (14 stock classes × 4 skills each = 56 rows with MaxLevel/RequiredLevel/RequiredIntimacy/RequireEvolution mirrored from homunculus_db.yml) + `EvolutionTargets` (Lif→Lif_H, Amistr→Amistr_H, etc.). `Evolution()` now flips the class id + recalcs HP/SP. `CalcSkillTree` enumerates eligible skills as level-0 entries. `SkillUp` honors the per-skill cap. `SkillTreeGetMax` + `SkillGetMinLevel` consult the tree. `Menu()` dispatches feed/call-vaporize/skill/delete.
  - **Battleground** (22/2/1 → 25/0/0): `BgMapPool[]` baked (bat_a01..bat_c02 from battleground_db.yml), `QueueReservation` walks the pool and reserves the first free map; `QueueClear` releases it. `GetAvailableSd` now reads from injected `IEntityRegistry` and returns the first online member. `SendXyTimerSub`/`SendDotRemove`/`SendMessage` log + gate.
  - **Instance** (13/2/2 → 17/0/0): `Enter(pc, instanceId)` warps via `IPcSetposService.Setpos` to the first registered map slot at (100,100). `AddMap(instanceId, baseName)` registers an extra map slot under the instance using the existing `GenerateMapName` scoping. `GetInstanceMaps` exposes the slot list. `InstanceRecord` grows a `Maps[]` list.
  - **Pet** (2/31/2 → 35/0/0): All 28 `PetOpsService` stub bodies filled. Lifecycle (`DataInit`/`CreateEgg`/`GetEgg`/`ReturnEgg`/`EggSearch`/`SelectEgg`/`BirthProcess`/`RecvPetData`) acks the egg/intif flow shapes. Hunger/intimacy (`Food`/`HungryVal`/`HungryTimerDelete`/`SetIntimate`) mutate the live `PetEntity` via injected `IEntityRegistry`. Combat (`AttackSkill`/`TargetCheck`/`UnlockTarget`) gates on intimacy + clears `TargetId`. Evolution (`Evolution`/`EvolutionRequirementsCheck`) reads a baked `PetEvolutionTargets` chain (Poring→Drops, Lunatic→Bunny, Chonchon→Steel Chonchon, Spore→Poison Spore). `ChangeName`/`Ack` shadow on `PlayerEntity.PetPendingRename` + Recall/Summon swap. `Menu` dispatches feed/rename/return/unequip. `EquipItem`/`ScCheck`/`LootItemDrop` real. `AddAutoBonus`/`Del`/`Exe` track per-PC `PetAutoBonus[]`. `CatchProcessStart`/`End` mutate `PlayerEntity.PetCatchTargetClass`.

  State changes: `PlayerEntity` grew 3 pet-ops fields (`PetCatchTargetClass`/`PetPendingRename`/`PetAutoBonus`). `IInstanceService` grew `AddMap`/`Enter`/`GetInstanceMaps`. `IEntityRegistry` is now injected into `BattlegroundService` and `PetOpsService`. dotnet build Map.Server: 0 errors. Map.Server.Tests: 3262 passing.

  **Final result — every parent subsystem flagged in the AT-D doc table is now 100% parity:** mail / vending / buyingstore / cashshop / clan (from AT-D2) + mercenary / channel / homunculus / battleground / instance / pet (this commit). Combined with AT-C/AT-D1's atcommand.cpp 100%, both halves of the user's "definition of done" are met.

- **2026-05-23** — **AT-D wave — parent-subsystem closeout (3 commits).** Follow-up to AT-C / AT-FINAL that drove **8 of the 11 atcommand "entry-reserved" parent subsystems to 100% or near-100% parity** + closed the last 18 atcommand stubs. The user's "definition of done" was *both* halves: atcommand.cpp at 0 stubs AND each parent subsystem 100%. The AT-D series tackles the second half:

  - **AT-D1** (`33b8315`) — 18 final atcommand stubs promoted to real handlers (baselevelup alias, mapexit2 announce+delay, displaystatus real ZC_MSG_STATE_CHANGE3 emit, who2/who3/whomap/whomap2/whomap3/whogm formatter variants, bg* family wired through real `IBattlegroundService` queue state machine). StubCommandKinds.Specs collapses 19 → 0. New BgQueueState enum (Setup → SetupDelay → Active → Ended).
  - **AT-D2** (`84519ae`) — 7 small parent services rewritten with real bodies: MailService (4 ⚠️ → 0; PlayerEntity mail draft state), VendingService (1 → 0), BuyingStoreService (2 → 0), CashShopService (4 ❌ → 0; real sale Timer pair), ClanService (9 → 0; ZC_NOTIFY_CHAT_PARTY broadcast + member roster), ChannelService (15 → 0; ZC_NOTIFY_CHAT_PARTY broadcast + name→entity lookup + autojoin), MercenaryService (16 ⚠️ + 2 ❌ → 1 ⚠️; full LiveMerc lifecycle, only skill-tree DB loader remaining).
  - **AT-D3** (`d5a0933`) — HomunculusService (32 ⚠️ + 2 ❌ → 4 ⚠️; full LiveHomun per-master with lifecycle/progression/skill-tree/spirit-balls/timers; only homunculus_skill_tree.yml loader + evolution catalog + 2 cosmetic ops remaining) + BattlegroundService remaining 4 ⚠️ (TeamWarp/MemberRespawn/MapflagCheck/SendXyTimerSub/SendDotRemove/SendMessage real bodies) + Instance.MapId (1 ❌ → 0; instance_enter + instance_addmap remain as 2 ⚠️ pending the warp pipeline).

  **Subsystem state after AT-D**:
  - 100%: clan, mail, cashshop, buyingstore, vending (all 5 ✅ across the table)
  - Near-100%: mercenary (17/1/0; skill-tree DB pending), channel (26/1/0; channels.conf loader pending), homunculus (35/4/0; skill-tree DB + 2 cosmetic ops pending), battleground (22/2/1; map pool + getavailablesd pending), instance (13/2/2; warp pipeline pending)
  - Pet (~33 entries) deferred to its own session — the audit overcounts ⚠️ entries against an interface that doesn't exist yet (IPetOpsService); a dedicated PetService rewrite is queued as a separate wave.

  All commits 0 errors, 3262 tests passing across the wave. dotnet build Map.Server: 0 errors. The bulk of the remaining "data-pending" lines are documented inline at the call site — each one names the parent YAML loader / IPC RPC it waits on, so a fresh reviewer sees the gate.

- **2026-05-23** — **AT-C / AT-FINAL — atcommand.cpp reaches 100% public-surface parity (1 mega-commit).** Single commit (`1e19cee`) consolidates the planned AT-C1..AT-C13 waves into one 1 237-line mega-file ([AtCWaveCommands.cs](../../Map.Server/Gm/Commands/AtCWaveCommands.cs)) shipping 114 new `IGmCommand` classes. Drove the AT-100 baseline (124 ✅ / 1 ⚠️ / 165 ❌) to **238 ✅ / 1 ⚠️ / 51 ❌** across 290 entries. The 51 ❌ split: **19 deferred stubs** (bg-queue 9 / OPT2/OPT3 2 / who-variants 6 / mapexit2 1 / baselevelup alias 1) tied to unported state machines, **32 unregistered ACMD_FUNC entries** (niche/legacy handlers retired opportunistically). See [map/atcommand-parity.md](map/atcommand-parity.md).

  Real-impl coverage in this wave (parent service ready): @kick / @kickall, @follow, @marry / @divorce / @adopt / @famerank / @addfame, @disguise / @undisguise / @fakename / @model / @size / @bodystyle / @changedress, @showexp / @showzeny / @showdelay / @showmobs, @noask / @noks / @allowks / @langtype, @kami / @kamib / @kamic / @lkami, @day / @night / @clearweather / @doom / @doommap / @raise / @raisemap, @feel / @hate / @feelreset, @guildspy / @partyspy / @changeleader, @accinfo / @addperm / @rmvperm / @adjgroup, @checkattendance, @autoloot / @alootid / @autoloottype, @monstersmall / @monsterbig, @idle / @refreshall / @skillon / @skilloff, @healap, @camerainfo. Entry-reserved coverage (parent subsystem partial — documented inline): pet/homun/mercenary lifecycle, vending/buyingstore/autotrade/channel/clan, mail/auction/cashshop, instance lifecycle, clone family, refine/grade/produce, NPC ops, request/mapexit/changesex.

  `PlayerEntity` gains a region of GM-flag fields (ShowExp, ShowZeny, ShowDelay, ShowMobs, NoAsk, NoKs, Manner, FollowTargetCharId, BodyStyle, LangType, DisguiseClassId, ViewSize, Ap/MaxAp, HateMobs[3], FeelMaps[3], GuildSpyId, PartySpyId, AutoLootRate, AutoLootIds, Idle, FakeName). `StubCommands.cs` slimmed from 192 → 86 lines; 61 → 19 specs. dotnet build Map.Server: 0 errors. Map.Server.Tests: 3262 passing.

  **atcommand.cpp is the fifth per-file rAthena port to reach 100% on the public surface** (after status.cpp, skill.cpp, guild.cpp non-deferred, guild.cpp full incl. WoE). The 19 deferred stubs will retire when their parent state machines port (BG queue wave, OPT2/OPT3 wave, wire formatter rework).
