...
- **2026-05-16** — **P8 — pre-gameplay cascade audit.** User reminded that the map server commits *all* gameplay state via char-side gRPC, so any half-wired RPC becomes a runtime dead end. Deep audit of every char_service RPC against rAthena `int_*.cpp` found 4 real gaps despite P1-P7 passing: PartyLeave didn't disband on leader exit; GuildBreak orphaned guild_skill/position/alliance/expulsion/storage/castle rows; MercenarySave dropped skill cooldowns; MercenaryDelete didn't cascade to cooldowns or mercenary_owner. All 4 closed with rAthena-faithful behavior + 8 new cascade regression tests. Suite at 148 + 16 = 164. Char-side gRPC surface is now genuinely complete; map gameplay won't hit dead ends.
- **2026-05-16** — **P7 complete.** Created `Login.Server.Tests` project with `LoginDataRepositoryTests` (global online registry mechanics), `LoginGrpcServiceCrossServerTests` (gRPC contract char servers depend on), and `AuthNodeReplayTests` (replay defense — deferred from P2.7). 16 login tests + 140 char tests = 156 total, all green. Doc sweep: every Pending section across all 7 migration docs is empty or explicitly deferred-to-gameplay. Soak/stress is a CI-infra concern out of scope here. **All seven phases done; pre-map parity surface is complete and locked in.**
- **2026-05-16** — **P6 complete.** Map → Char infrastructure-level wiring done. Periodic timers (registration/keep-alive/user-count/autosave) in `MapServerImpl`. `EnterMap`/`LeaveMap` wired to lifecycle RPCs. Shutdown batch-saves and clears online flags. Map-side `ForceDisconnectAccount` handler added; char-side cascades the kick. Module-RPC wrappers ready without triggers per scope. **P1–P6 done.** Next: P7 (multi-server integration harness).
